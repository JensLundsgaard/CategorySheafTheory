...
\section{Direct Product} \label{direct_product}
For a family $\{(A_\alpha, +_\alpha)\}_{\alpha \in I}$ of vector spaces, abelian groups $A_\alpha$, etc. with the commutative operation $+_\alpha$ the direct sum $\otimes_{\alpha \in I} A_\alpha$ is defined as the set of tuples $(a_\alpha)_{\alpha \in I}$ where each component $a_\alpha \in A_\alpha$ and if $(b_\alpha)_\{\alpha \in I\}$ is another such tuple, then their sum
$$ (a_\alpha)_{\alpha \in I} + (b_\alpha)_\{\alpha \in I\} = (a_\alpha +_\alpha b_\alpha)_{\alpha \in I}$$

\section{Cellular Sheaves} \label{\cellular_sheaves}
A cellular sheaf $F$ on a simplicial complex $X$ consists of the following:
\begin{itemize}
    \item The face or boundary relation that $\delta$ is a subsimplex of $\tau$ is $\delta \leq \tau$. 
    \item Each simplex $\delta$ receives a vector space $F(\delta)$ and $F$ acts functorially on the face morphism 
     $$\delta \leq \tau \mapsto F(\delta \leq \tau): F(\delta) \to F(\tau)$$
    where $F(\delta \leq \tau)$ is a linear mapping from $F(\delta)$ to $F(\tau)$. 
\end{itemize}
We write $F(\delta \leq \tau) = F_{\delta \leq \tau}$. (Wei et al. also give the axioms $\rho \leq \delta \leq \tau \Rightarrow F_{\rho \leq \tau} = F_{\delta \leq \tau} \circ F_{\rho \leq \delta}$ and $F_{\delta \leq \delta} = \text{id}_{F(\delta)}$, but these are both necessary for functoriality).\\

\section{Cellular Pullback Sheaves} \label{cellular_pullback_sheaves}
If $f:X \to Y$ is a simplicial map and $F$ is a cellular sheaf on $Y$, then the pullback sheaf $f^* F$ is given by
$$(f^* F)(\delta) = F(f(\delta))$$
and for the face relation $\delta \leq \tau$
$$(f^* F)_{\delta \leq \tau} = F_{f(\delta) \leq f(\tau)}$$

\section{Cellular Sheaf Cohomology} \label{cell_sheaf_cohomology}
Let $F$ be a sheaf on a simplicial complex $X$ then  TODO finish this.

\section{Monoid Object in a Monoidal Category} \label{monoid_object}
Let a monoidal category $(\cat{C}, \otimes, I)$ then the tuple $(M,\eta, \mu)$ where 
$$M \in \text{ob}(\cat{C})$$
$$M \otimes M \arr{\mu} M$$
$$I \arr{\eta} M$$
Satisfying
\diag{
\& (M \otimes M) \otimes M \arrow[r, "\alpha"] \arrow[d, ""]\& M \otimes (M \otimes M) \arrow[r, "\text{id}_{M} \otimes \mu"] \& M \otimes M \arrow[d, "\mu"] \\
\& M \otimes M \arrow[r, "\mu X"]\& T(X)
}

\diag{
\& T(X) \arrow[r, "\eta T(X)"] \arrow[rd, equal] \arrow[d, "T (\eta X)"]\& T(T(X)) \arrow[d, "\mu X"] \\
\& T(T(X)) \arrow[r, "\mu X"]\& T(X)
}


\section{Monads} \label{monads}
Let a category $\cat{C}$, then a monad is the triple
$$(T, \eta, \mu)$$
where
$$T: \cat{C} \to \cat{C} \text{ is an endofunctor}$$
$$\text{id}_\cat{C} \arr{\eta} T \text{ is a natural transformation from the identity on $\cat{C}$ to $T$}$$
$$T \circ T \arr{\mu} T \text{ is a natural transformation}$$
and making the following diagram commute: 
\diag{
\& T(T(T(X))) \arrow[r, "T(\mu X)"] \arrow[d, "\mu T(X)"]\& T(T(X)) \arrow[d, "\mu X"] \\
\& T(T(X)) \arrow[r, "\mu X"]\& T(X)
}
and
\diag{
